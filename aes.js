		...
		expandedKey.slice(Nb*Nr));return unpackBytes(block);}function decryptOld(block,
		...
		block--){aBlock=decryptOld(ciphertext.slice(block*bpb,(block+1)*bpb),expandedKey);
		if(mode=="CBC")for(var i=0;i<bpb;i++)pt[(block-1)
		*bpb+i]=aBlock[i]^ ciphertext[(block-1)*bpb+i];else pt=aBlock.concat(pt);}
		if(mode=="ECB")pt=decryptOld(ciphertext.slice(0,bpb),expandedKey).concat(pt);
		return pt;}
		...
		var maxLineLength=50;var hexSentinel="?HX?",hexEndSentinel="?H";
		...
		base64esent="?64b",base64addsent=false;function armour_base64(b){var b64t="";
		var b64l=base64addsent?base64sent:"";var i;for(i=0;i<=b.length-3;i+=3)
		{if((b64l.length+4)>maxLineLength){b64t+=b64l+lineBreak;b64l="";}
		...
		var loadTime=(new Date()).getTime();var key;var prng;function setKey()
		{var s=encode_utf8(document.key.text.value);
		var i,kmd5e,kmd5o;if(s.length==1){s+=s;}md5_init();for(i=0;i<s.length;i+=2)
		...
		hs=byteArrayToHex(key);}function Generate_key(){var i,j,k="";
		...
		document.key.text.value=k;delete prng;}function Encrypt_text(){var v,i;
		if(document.key.text.value.length==0)
		{alert("Please specify a key with which to encrypt the message.");return;}if(document.plain.text.value.length==0){alert("No plain text to encrypt!  Please enter or paste plain text in the field above.");
		return;}
		maxLineLength = document.plain.maxLineLength.value;
		lineBreak = "";
		if (document.plain.EOL[1].checked)
			lineBreak = "<br />";
		else if (document.plain.EOL[2].checked)
			lineBreak = "\\n";
		if (!document.plain.singleLine.checked)
			lineBreak += "\n";
		document.cipher.text.value="";setKey();addEntropyTime();
		prng=new AESprng(keyFromEntropy());
		var plaintext=encode_utf8(document.plain.text.value);md5_init();for(i=0;
		i<plaintext.length;i++){md5_update(plaintext.charCodeAt(i));}md5_finish();
		...
		v=armour_base64(ct);
		document.cipher.text.value=v;delete prng;}
		function Decrypt_text(){if(document.key.text.value.length==0){alert("Please specify a key with which to decrypt the message.");
		return;}if(document.cipher.text.value.length==0){alert("No cipher text to decrypt!  Please enter or paste cipher text in the field above.");return;}
		//document.plain.text.value="";
		setKey();var ct=new Array();
		ct=disarm_base64(document.cipher.text.value.replace(/<br>/gi, "").replace(/<br \/>/gi, "").replace(/\\n/gi, ""));
		var result=rijndaelDecrypt(ct,key,"CBC");var header=result.slice(0,20);result=result.slice(20);
		var dl=(header[16]<<24)|(header[17]<<16)|(header[18]<<8)|header[19];if((dl<0)
		||(dl>result.length)){//alert("Message (length "+result.length+") truncated.  "+dl+" characters expected.");
		dl=result.length;}var i,plaintext="";md5_init();for(i=0;i<dl;i++)
		{plaintext+=String.fromCharCode(result[i]);md5_update(result[i]);}md5_finish();successful = true;
		for(i=0;i<digestBits.length;i++){if(digestBits[i]!=header[i]){alert("Invalid decryption key.");successful = false;
		break;}}
		if (successful) {
			//document.plain.text.value=decode_utf8(plaintext);
			document.plain.text.value=decode_utf8(plaintext.replace(/\n/g,'<br \/>'));
		}}
